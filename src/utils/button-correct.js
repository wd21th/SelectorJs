...
function buttonCorrect (item) {
  ...
  if (keys.includes('id')) {
    ...
    let varable = `button${varableName} = document.querySelector('${item.tagName}#${buttonId}')`;
    arr.push(varable);
  } else if (keys.includes('type')) {
    let varableName = checkIdVarName(item.attrs['type']);
    let buttonType = item.attrs['type'];

    let varable = `button${varableName} = document.querySelector('${item.tagName}[type="${buttonType}"]')`;
    arr.push(varable);
  } else if (keys.includes('value')) {
    let varableName = checkIdVarName(item.attrs['value']);
    let buttonValue = item.attrs['value'];

    let varable = `button${varableName} = document.querySelector('${item.tagName}[value="${buttonValue}"]')`;
    arr.push(varable);
  } else if (keys.includes('name')) {
    let varableName = checkIdVarName(item.attrs['name']);
    let buttonName = item.attrs['name'];

    let varable = `button${varableName} = document.querySelector('${item.tagName}[name="${buttonName}"]')`;
    arr.push(varable);
  } else {
